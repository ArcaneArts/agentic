...
const _2 _S=['_subclass_Content','audioUrl','base64Audio','contents','AudioContent','ContentGroup','ImageContent','TextContent','imageUrl','base64Image','text','content','_subclass_Message','UserMessage','SystemMessage','AgentMessage','ToolMessage','Missing required Message."content" in map ','Missing required UserMessage."content" in map ','Missing required SystemMessage."content" in map ','toolCalls','Missing required AgentMessage."content" in map ','toolCallId','Missing required ToolMessage."content" in map ','Missing required ToolMessage."toolCallId" in map ','displayName','cost','capabilities','deprecated','Missing required ChatModel."id" in map ','Missing required ChatModel."cost" in map ','Missing required ChatModel."capabilities" in map ','messages','tools','model','systemPrompt','user','responseFormat','Missing required ChatRequest."messages" in map ','Missing required ChatRequest."model" in map ','message','realCost','finishReason','metadata','usage','Missing required ChatResult."message" in map ','Missing required ChatResult."realCost" in map ','inputTokens','outputTokens','name','arguments','Missing required ToolCall."id" in map ','Missing required ToolCall."name" in map ','Missing required ToolCall."arguments" in map ','description','schema','Missing required ToolSchema."name" in map ','Missing required ToolSchema."description" in map ','Missing required ToolSchema."schema" in map ','ultraCompatibleMode','systemMode','contextWindow','maxTokenOutput','inputModalities','outputModalities','reasoning','structuredOutput','streaming','seesToolMessages','Missing required ChatModelCapabilities."tools" in map ','Missing required ChatModelCapabilities."ultraCompatibleMode" in map ','Missing required ChatModelCapabilities."systemMode" in map ','Missing required ChatModelCapabilities."contextWindow" in map ','Missing required ChatModelCapabilities."maxTokenOutput" in map ','Missing required ChatModelCapabilities."inputModalities" in map ','Missing required ChatModelCapabilities."outputModalities" in map ','Missing required ChatModelCapabilities."reasoning" in map ','Missing required ChatModelCapabilities."structuredOutput" in map ','Missing required ChatModelCapabilities."streaming" in map ','Missing required ChatModelCapabilities."seesToolMessages" in map ','input','output','Missing required ChatModelCost."input" in map ','Missing required ChatModelCost."output" in map '];const _13 _V=[<_8>[],<_k>[],false,<_t>[],ChatFinishReason.unspecified,<_3,_4>{},ChatUsage()];const _o _T=true;const _o _F=false;const _5 _ = 0;
...
extension $ChatModel on _g{
  ...
  static _g fromMap(_1 r){_;_1 m=r.$nn;return _g(id: m.$c('id')? _0.da(m['id'], _3) as _3:(throw _r('${_S[29]}$m.')),displayName: m.$c(_S[25]) ?  _0.da(m[_S[25]], _3) as _3? : null,cost: m.$c(_S[26])?$ChatModelCost.fromMap((m[_S[26]]) as _1):(throw _r('${_S[30]}$m.')),capabilities: m.$c(_S[27])?$ChatModelCapabilities.fromMap((m[_S[27]]) as _1):(throw _r('${_S[31]}$m.')),deprecated: m.$c(_S[28]) ?  _0.da(m[_S[28]], _o) as _o : _V[2],);}
  _g copyWith({_3? id,_3? displayName,_o deleteDisplayName=_F,_n? cost,_m? capabilities,_o? deprecated,_o resetDeprecated=_F,})=>_g(id: id??_H.id,displayName: deleteDisplayName?null:(displayName??_H.displayName),cost: cost??_H.cost,capabilities: capabilities??_H.capabilities,deprecated: resetDeprecated?_V[2]:(deprecated??_H.deprecated),);
}
extension $ChatRequest on _h{
  ...
  static _h fromMap(_1 r){_;_1 m=r.$nn;return _h(messages: m.$c(_S[32])? (m[_S[32]] as _p).$m((e)=>$Message.fromMap((e) as _1)).$l:(throw _r('${_S[38]}$m.')),tools: m.$c(_S[33]) ?  (m[_S[33]] as _p).$m((e)=> _0.da(e, _t) as _t).$l : _V[3],model: m.$c(_S[34])?$ChatModel.fromMap((m[_S[34]]) as _1):(throw _r('${_S[39]}$m.')),systemPrompt: m.$c(_S[35]) ?  _0.da(m[_S[35]], _3) as _3? : null,user: m.$c(_S[36]) ?  _0.da(m[_S[36]], _3) as _3? : null,responseFormat: m.$c(_S[37]) ? $ToolSchema.fromMap((m[_S[37]]) as _1) : null,);}
  _h copyWith({_u? messages,_u? appendMessages,_u? removeMessages,_v? tools,_o resetTools=_F,_v? appendTools,_v? removeTools,_g? model,_3? systemPrompt,_o deleteSystemPrompt=_F,_3? user,_o deleteUser=_F,_l? responseFormat,_o deleteResponseFormat=_F,})=>_h(messages: (messages??_H.messages).$u(appendMessages,removeMessages),tools: ((resetTools?_V[3]:(tools??_H.tools)) as _v).$u(appendTools,removeTools),model: model??_H.model,systemPrompt: deleteSystemPrompt?null:(systemPrompt??_H.systemPrompt),user: deleteUser?null:(user??_H.user),responseFormat: deleteResponseFormat?null:(responseFormat??_H.responseFormat),);
}
extension $ChatResult on _i{
  _i get _H=>this;
  _3 toJson({bool pretty=_F})=>_0.j(pretty, toMap);
  _1 toMap(){_;return <_3, _4>{_S[40]:message.toMap(),_S[41]:_0.ea(realCost),_S[42]:finishReason.name,_S[43]:metadata.$m((k,v)=>_w(k,v)),_S[44]:usage.toMap(),}.$nn;}
  static _i fromJson(String j)=>fromMap(_0.o(j));
  static _i fromMap(_1 r){_;_1 m=r.$nn;return _i(message: m.$c(_S[40])?$AgentMessage.fromMap((m[_S[40]]) as _1):(throw _r('${_S[45]}$m.')),realCost: m.$c(_S[41])? _0.da(m[_S[41]], _y) as _y:(throw _r('${_S[46]}$m.')),finishReason: m.$c(_S[42]) ? _0.e(ChatFinishReason.values, m[_S[42]]) as ChatFinishReason : _V[4],metadata: m.$c(_S[43]) ?  _0.fe((m[_S[43]] as Map).$e.$m((e)=>_x(e.key,e.value))) : _V[5],usage: m.$c(_S[44]) ? $ChatUsage.fromMap((m[_S[44]]) as _1) : _V[6],);}
  _i copyWith({_e? message,_y? realCost,_z? finishReason,_o resetFinishReason=_F,_1? metadata,_o resetMetadata=_F,_j? usage,_o resetUsage=_F,})=>_i(message: message??_H.message,realCost: realCost??_H.realCost,finishReason: resetFinishReason?_V[4]:(finishReason??_H.finishReason),metadata: resetMetadata?_V[5]:(metadata??_H.metadata),usage: resetUsage?_V[6]:(usage??_H.usage),);
}
...
extension $ToolCall on _k{
  ...
  static _k fromMap(_1 r){_;_1 m=r.$nn;return _k(id: m.$c('id')? _0.da(m['id'], _3) as _3:(throw _r('${_S[51]}$m.')),name: m.$c(_S[49])? _0.da(m[_S[49]], _3) as _3:(throw _r('${_S[52]}$m.')),arguments: m.$c(_S[50])? _0.da(m[_S[50]], _3) as _3:(throw _r('${_S[53]}$m.')),);}
...
extension $ToolSchema on _l{
  _l get _H=>this;
  _3 toJson({bool pretty=_F})=>_0.j(pretty, toMap);
  _1 toMap(){_;return <_3, _4>{_S[49]:_0.ea(name),_S[54]:_0.ea(description),_S[55]:schema.$m((k,v)=>_w(k,v)),}.$nn;}
  static _l fromJson(String j)=>fromMap(_0.o(j));
  static _l fromMap(_1 r){_;_1 m=r.$nn;return _l(name: m.$c(_S[49])? _0.da(m[_S[49]], _3) as _3:(throw _r('${_S[56]}$m.')),description: m.$c(_S[54])? _0.da(m[_S[54]], _3) as _3:(throw _r('${_S[57]}$m.')),schema: m.$c(_S[55])? _0.fe((m[_S[55]] as Map).$e.$m((e)=>_x(e.key,e.value))):(throw _r('${_S[58]}$m.')),);}
...
extension $ChatModelCapabilities on _m{
  ...
  static _m fromMap(_1 r){_;_1 m=r.$nn;return _m(tools: m.$c(_S[33])? _0.da(m[_S[33]], _o) as _o:(throw _r('${_S[69]}$m.')),ultraCompatibleMode: m.$c(_S[59])? _0.da(m[_S[59]], _o) as _o:(throw _r('${_S[70]}$m.')),systemMode: m.$c(_S[60])?_0.e(ChatModelSystemMode.values, m[_S[60]]) as ChatModelSystemMode:(throw _r('${_S[71]}$m.')),contextWindow: m.$c(_S[61])? _0.da(m[_S[61]], _5) as _5:(throw _r('${_S[72]}$m.')),maxTokenOutput: m.$c(_S[62])? _0.da(m[_S[62]], _5) as _5:(throw _r('${_S[73]}$m.')),inputModalities: m.$c(_S[63])? (m[_S[63]] as _p).$m((e)=>_0.e(Modality.values, e) as Modality).$l:(throw _r('${_S[74]}$m.')),outputModalities: m.$c(_S[64])? (m[_S[64]] as _p).$m((e)=>_0.e(Modality.values, e) as Modality).$l:(throw _r('${_S[75]}$m.')),reasoning: m.$c(_S[65])? _0.da(m[_S[65]], _o) as _o:(throw _r('${_S[76]}$m.')),structuredOutput: m.$c(_S[66])? _0.da(m[_S[66]], _o) as _o:(throw _r('${_S[77]}$m.')),streaming: m.$c(_S[67])? _0.da(m[_S[67]], _o) as _o:(throw _r('${_S[78]}$m.')),seesToolMessages: m.$c(_S[68])? _0.da(m[_S[68]], _o) as _o:(throw _r('${_S[79]}$m.')),);}
...
extension $ChatModelCost on _n{
  ...
  static _n fromMap(_1 r){_;_1 m=r.$nn;return _n(input: m.$c(_S[80])? _0.da(m[_S[80]], _12) as _12:(throw _r('${_S[82]}$m.')),output: m.$c(_S[81])? _0.da(m[_S[81]], _12) as _12:(throw _r('${_S[83]}$m.')),);}
